...
class Head(nn.Module):
    ...
    def forward(self, motion_feature, x, flow): # /16 /8 /4
        ...
        motion_feature=motion_feature.float()
        x = x.float()
...
class TimeBlock(nn.Module):
    def __init__(self,out_channels):
        ...
        self.timeconvblock=nn.Sequential(conv(in_channels, in_channels),
                                         conv(in_channels, out_channels*2, kernel_size=1, padding=0) 
                                        )
        self.timeconvblock2=nn.Sequential(conv(in_channels, in_channels),
                                         conv(in_channels, out_channels*2, kernel_size=1, padding=0) 
                                        )
        '''self.timeconvblock3=nn.Sequential(conv(in_channels, in_channels),
                                         conv(in_channels, out_channels*2, kernel_size=1, padding=0) 
                                        )'''
    def forward(self,t,af):
        # print(t.shape,af.shape)
        _,in_ch,_,_=t.shape
        # print(t.shape,af.shape)
        out = self.timeconvblock(torch.cat([t,af],1))
        # print(t.shape,out[:,in_ch:].shape,out[:,0:in_ch].shape)
        t = t.clone()*out[:,in_ch:]+out[:,0:in_ch]
        out = self.timeconvblock2(torch.cat([t,af],1))
        t = t.clone()*out[:,in_ch:]+out[:,0:in_ch]
        # out = self.timeconvblock3(torch.cat([t,af],1))
        # t = t.clone()*out[:,in_ch:]+out[:,0:in_ch]
        return t
...
class MultiScaleFlow(nn.Module):
    def __init__(self, backbone, **kargs):
        ...
        self.time_block=nn.ModuleList([TimeBlock(out_channels=kargs['embed_dims'][-1-i]) for i in range(self.flow_num_stage)])
        self.deconv_block = nn.ModuleList([deconv(kargs['embed_dims'][-1-i], kargs['embed_dims'][-2-i]) for i in range(self.flow_num_stage-1)])
    ...
    def calculate_flow(self, imgs, timestep, af=None, mf=None):
        ...
        timestep=torch.unsqueeze(timestep,dim=2)
        timestep=torch.unsqueeze(timestep,dim=3)
        ...
        for i in range(self.flow_num_stage):
            if t == None:
                _,c,h,w = af[-1-i][:B].shape
                t = timestep.expand(timestep.size(0),c,h,w).cuda().clone().float()
            else:
                t = self.deconv_block[i-1](t)
            t = self.time_block[i](t,torch.cat([af[-1-i][:B],af[-1-i][B:]],1))
            if flow != None:
