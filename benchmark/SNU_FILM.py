...
for test_file in level_list:
    ...
    file_list = []
    i=i+1
    save_path = os.makedirs(str(i), exist_ok=True)
    ...
    for line in tqdm(file_list):
        ...
        I0_path = rf'{os.path.join(path, line[0])}'
        I1_path = os.path.join(path, line[1])
        I2_path = os.path.join(path, line[2])
        # print(I0_path,I1_path,I2_path)
        ...
        # I1_pred = model.inference(I0, I2, TTA,fast_TTA=TTA)[0]
        # print(I1_pred.shape) torch.Size([3, 736, 1280])
        I1_pred = padder.unpad(I1_pred)
        # print(I1_pred.shape) torch.Size([3, 720, 1280])
        # print(I1.shape) torch.Size([1, 3, 720, 1280])
        ...
        cv2.imwrite(str(i)+'/'+str(j)+".png", (I1_*255).astype(np.uint8))
        cv2.imwrite(str(i)+'/'+str(j)+'_pred.png',(I1_pred*255).astype(np.uint8))
        # print(str(i)+'/'+str(j)+".png")
        j=j+1
