...
for strMode in ['XTEST-2k', 'XTEST-4k']:
    i=0
    os.makedirs(strMode, exist_ok=True)
    fltPsnr, fltSsim = [], []
    path = ''
    for intFrame in tqdm(listFiles):
        # print(intFrame[0],intFrame[1],intFrame[2])
        ...
        # print(tenOne.shape,tenTwo.shape,intFrame[3])
        tenEstimate, flow = model.hr_inference(tenOne, tenTwo, TTA = TTA, timestep=intFrame[3], down_scale = down_scale)

        tenEstimate = padder.unpad(tenEstimate[0])
        # flow = padder.unpad(flow[0])
        # flow_im1 = flow_to_image(flow[0:2])
        # flow_im2 = flow_to_image(flow[2:4])
        # flow_im1 = flow_im1.detach().cpu().numpy().transpose(1, 2, 0)
        # flow_im2 = flow_im2.detach().cpu().numpy().transpose(1, 2, 0)
        ...
        # if (i)%7==0:
        #     os.makedirs(strMode+'/'+str(i//7), exist_ok=True)
        #     path = strMode+'/'+str(i//7)
        #     img0 = cv2.imread(intFrame[0])
        #     img1 = cv2.imread(intFrame[1])
        #     cv2.imwrite(path+intFrame[0][-9:], img0)
        #     cv2.imwrite(path+intFrame[1][-9:], img1)
        i+=1

        # cv2.imwrite(path+intFrame[2][-9:], npyEstimate)
        # cv2.imwrite(path+'/flow1_'+intFrame[2][-8:], flow_im1)
        # cv2.imwrite(path+'/flow2_'+intFrame[2][-8:], flow_im2)
