...
class Model:
    ...
    def load_model(self, epoch=0, rank=0):
        def convert(param):
            return {
            k.replace("module.", ""): v
        ...
        if rank <= 0 :
            self.net.load_state_dict(convert(torch.load('ckpt/'+str(epoch)+'.pkl')))# 
    
    def save_model(self, rank=0,epoch=0):
        if rank == 0:
            torch.save(self.net.state_dict(),'ckpt/'+str(epoch)+'.pkl')
    def save_best_model(self, rank=0,epoch=0):
        if rank == 0:
            torch.save(self.net.state_dict(),'ckpt/'+str(epoch)+'best.pkl')
    ...
    def hr_inference(self, img0, img1, TTA = False, down_scale = 1.0, timestep = 0.5, fast_TTA = False):
        ...
        def infer(imgs):
            ...
            if fast_TTA:
                flow, mask = self.net.calculate_flow(imgs_down, timestep=torch.Tensor([timestep,timestep]))
            else:
                flow, mask = self.net.calculate_flow(imgs_down, timestep=timestep)
        ...
        if TTA == False:
            ...
            return (infer(imgs)[0] + infer(imgs.flip(2).flip(3))[0].flip(2).flip(3)) / 2, (infer(imgs)[1] + infer(imgs.flip(2).flip(3))[1].flip(2).flip(3)) / 2
    ...
    def inference(self, img0, img1, TTA = False, timestep = 0.5, fast_TTA = False):
        ...
        if fast_TTA:
            imgs_ = imgs.flip(2).flip(3)
            input = torch.cat((imgs, imgs_), 0)
            _, _, _, preds = self.net(input, timestep=torch.Tensor([timestep,timestep]))
        ...
        if TTA == False:
            ...
            _, _, _, pred2 = self.net(imgs.flip(2).flip(3), timestep=torch.Tensor([timestep,timestep]))
